...
################################################################################
  function bootstrapwp_css_loader() {
    ...
    wp_enqueue_style('screen', get_template_directory_uri().'/css/screen.css', false ,'20.4', 'all' );
...
/*
  function bootstrapwp_js_loader() {
       ...
       wp_enqueue_script('site_js', get_template_directory_uri().'/js/site_js.js', array('jquery'),'3.5', true );
  }
add_action('wp_enqueue_scripts', 'bootstrapwp_js_loader');
*/

add_action("wp_enqueue_scripts", "my_jquery_enqueue", 15);
 function my_jquery_enqueue() {
//    wp_deregister_script('jquery');
   wp_register_script('jquery', "//code.jquery.com/jquery-3.2.1.slim.min.js", false, null);
   wp_enqueue_script('jquery');
   wp_enqueue_script('site_js', get_template_directory_uri().'/js/site_js.js', array('jquery'),'4.3', true );
}
...
function exclude_category( &$wp_query ) 
   ...
   { if( is_home() || ( is_feed() && !is_category() ) || ( is_archive() && !is_category() )) { // Exclude from home, feed, but not from category page/feed
        set_query_var('category__not_in', array(11)); // Exclude category with ID 120
    }
}
*/
...
/*
if ( ! function_exists( 'create_post_type', $args ) ) :

function create_post_type() {
	
	// You'll want to replace the values below with your own.
	register_post_type( 'guide_network', // change the name
		array(
			'labels' => array(
				'name' => __( 'Guide Network' ), // change the name
				'singular_name' => __( 'guide_network' ), // change the name
			),
			'public' => true,
			...
			'has_archive' => true,
// 			'rewrite' => array ( 'slug' => __( 'guide_network' ) ) // change the name
		)
	);
...
/*
add_action( 'init', 'guide_network_taxonomy', 0 );
 
//create a custom taxonomy name it subjects for your posts
 
function guide_network_taxonomy() {
...
// Now register the taxonomy
  register_taxonomy('regions',array('guide_network'), array(
    'hierarchical' => true,
    'has_archive' => true,
    'labels' => $labels,
    'show_ui' => true,
    ...
    'query_var' => true,
    'rewrite' => array( 'slug' => 'guide_network' ),
  ));
